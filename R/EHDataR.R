...
EHTheme <- function(){
  ...
  x <- theme(axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 9), axis.text.x = element_blank(), axis.ticks.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(), panel.grid.major.y=element_line(color="gray"), panel.background = element_rect(fill = "lightskyblue2", color="darkslategray"))
...
{  
  ...
  for(i in 1:ncol(df)) {   
    ...
    p <- eval(substitute(ggplot(dfBar3, aes(x=Selection, y=Count, fill="steelblue1")) +
                           coord_flip() +
                           geom_col() +
                           theme(legend.position="none") +
                           ggtitle(colnames(df)[i]) +
                           theme(title = element_text(size =(font_size), face="bold"), axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.x = element_text(size = font_size), axis.text.y = element_text(size = font_size), axis.ticks.x = element_blank(), axis.ticks.y = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(), panel.grid.major.y=element_line(color="slategray2"), panel.background = element_rect(fill = "slategray2")) +
                           geom_text(aes(label = Count), size=(3), fontface="bold", color="black",
                                     hjust = 1.5), list(i=i)))
...
EHExplore_OneContinuousAndOneCategoricalColumn_Boxplots <- function(df, y, yCategorical=TRUE)
{
  ...
  for(i in 1:zz) {
    ...
    p <- ggplot(df, aes_string(x1, y1, fill=y1)) +
      #xlab(colnames(df)[i])  +
      #ylab(xText) +
      theme(title = element_text(size=9), axis.title.x = element_text(size = 9), axis.title.y = element_text(size = 9), panel.grid.major.x = element_blank(), panel.grid.minor.x=element_blank(), panel.grid.minor.y=element_blank(), panel.grid.major.y=element_line(color="gray"), panel.background = element_rect(fill = "lightskybue1", color="darkslategray")) +
      scale_color_d3()+
      scale_fill_d3()+   
      theme(legend.position = "none") +
      ggtitle(colnames(df)[i]) +
