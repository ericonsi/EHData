...
EHPrepare_RemoveColumnsWithAllNA <- function(df)
{
  ...
  EHSummarize_CategoryByTotal_ReturnsSingleTable <- function(df, font_size=7, decreasingOrder=TRUE, rectfill="slategray2", xfill = "ivory", xtitle = "", labelSize = 4, labelColor="blue")
  {
    ...
    p <- a1 +
      ...
      geom_text(aes(label = scales::comma(Total)), size=(labelSize), fontface="bold", color=labelColor, hjust = -.2) +
